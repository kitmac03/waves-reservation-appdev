...
class PaymentController extends Controller
{
    public function processPayment(Request $request)
    {
        ...
        try {
            ...
            $reservation = Reservation::with(['bill', 'customer'])->findOrFail($request->reservation_id);
            ...
            return redirect()->route('admin.vendor.reservation_calendar')->with(['success' => 'Downpayment verified successfully.']);
    ...
    public function processWalkin(Request $request)
    {
        $request->validate([
            'payment_amount' => 'required|numeric|min:0',
            'reservation_id' => 'required|string|regex:/^RES-\d{8}\d{3}$/',
        ...
        try {
            ...
            return redirect()->route('admin.vendor.reservation_calendar')->with(['success' => 'Reservation created successfully.']);
    ...
    public function processRemBal(Request $request)
    {
        $request->validate([
            'payment_amount' => 'required|numeric|min:0',
            'reservation_id' => 'required|string|regex:/^RES-\d{8}\d{3}$/',
        ...
        try {
            ...
            return redirect()->route('admin.vendor.reservation_calendar')->with(['success' => 'Payment successful.']);
    ...
    public function invalidPayment(Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()
                ->with('error', 'Downpayment marked as invalid.')
