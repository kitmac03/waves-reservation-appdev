...
<body class="bg-gray-100 p-6">
    <div class="max-w-5xl mx-auto bg-white p-6 rounded-lg shadow-lg">
        <h1 class="text-2xl font-bold text-center mb-6">Table Management</h1>
                ...
                <tbody>
                    @foreach($tables as $table)
                        <tr class="{{ $table->is_active ? 'active' : 'archived' }}" 
                            ...
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $table->id }}</td>
                            <td class="border border-gray-300 px-4 py-2">{{ $table->name }}</td>
                            ...
                            <td class="border border-gray-300 px-4 py-2 text-center">
                                @if($table->is_active)
                                    ...
                                    <!-- Archive Button -->
                                    <form action="{{ route('tables.archive', $table->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('PATCH')
                                ...
                                @else
                                    <!-- Unarchive Button -->
                                    <form action="{{ route('tables.unarchive', $table->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('PATCH')
